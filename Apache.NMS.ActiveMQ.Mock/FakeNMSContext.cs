...
internal class FakeNMSContext : INMSContext
{
    ...
    public INMSConsumer CreateConsumer(IDestination destination)
    {
        ...
        return new FakeNMSConsumer(queueName, null);
    }
    public INMSConsumer CreateConsumer(IDestination destination, string selector)
    {
        var queueName = destination.ToString();
        return new FakeNMSConsumer(queueName, msg => string.IsNullOrEmpty(selector) || msg.Body.Contains(selector));
    }
    ...
    public INMSConsumer CreateDurableConsumer(ITopic topic, string name) => new FakeNMSConsumer(topic.ToString(), null, durable: true, durableName: name);
    public INMSConsumer CreateDurableConsumer(ITopic topic, string name, string selector) => new FakeNMSConsumer(topic.ToString(), msg => string.IsNullOrEmpty(selector) || msg.Body.Contains(selector), durable: true, durableName: name);
    public INMSConsumer CreateDurableConsumer(ITopic topic, string name, string selector, bool noLocal) => new FakeNMSConsumer(topic.ToString(), msg => string.IsNullOrEmpty(selector) || msg.Body.Contains(selector), durable: true, durableName: name);
    public INMSConsumer CreateSharedConsumer(ITopic topic, string name) => new FakeNMSConsumer(topic.ToString(), null, shared: true, sharedName: name);
    public INMSConsumer CreateSharedConsumer(ITopic topic, string name, string selector) => new FakeNMSConsumer(topic.ToString(), msg => string.IsNullOrEmpty(selector) || msg.Body.Contains(selector), shared: true, sharedName: name);
    public INMSConsumer CreateSharedDurableConsumer(ITopic topic, string name) => new FakeNMSConsumer(topic.ToString(), null, durable: true, durableName: name, shared: true, sharedName: name);
    public INMSConsumer CreateSharedDurableConsumer(ITopic topic, string name, string selector) => new FakeNMSConsumer(topic.ToString(), msg => string.IsNullOrEmpty(selector) || msg.Body.Contains(selector), durable: true, durableName: name, shared: true, sharedName: name);
    ...
    public ITemporaryQueue CreateTemporaryQueue() => new FakeTemporaryQueue(Guid.NewGuid().ToString());
    public Task<ITemporaryQueue> CreateTemporaryQueueAsync() => Task.FromResult<ITemporaryQueue>(CreateTemporaryQueue());
    public ITemporaryTopic CreateTemporaryTopic() => new FakeTemporaryTopic(Guid.NewGuid().ToString());
