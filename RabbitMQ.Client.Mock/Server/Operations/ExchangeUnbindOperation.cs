...
internal class ExchangeUnbindOperation(IRabbitServer server, string source, string destination, string routingKey, IDictionary<string, object?>? arguments = null) : Operation<object>(server)
{
    ...
    public override ValueTask<OperationResult<object>> ExecuteAsync(CancellationToken cancellationToken)
    {
        ...
        { 
            ...
            if (binding is null)
            {
                return ValueTask.FromResult(OperationResult.Failure<object>($"Binding '{routingKey}' not found."));
            }

            // remove the target exchange from the binding
            if (!binding.BoundExchanges.Remove(destination))
            {
                return ValueTask.FromResult(OperationResult.Success<object>($"Exchange '{destination}' has already been unbound from exchange '{source}' with key '{routingKey}'."));
            }
            ...
            if (binding.BoundExchanges.Count == 0)
            {
                ...
                return ValueTask.FromResult(OperationResult.Success<object>($"Binding '{routingKey}' removed."));
            }

            // the exchange binding was removed. return success.
            return ValueTask.FromResult(OperationResult.Success<object>($"Exchange '{destination}' unbound from exchange '{source}' with key '{routingKey}'."));
        }
        catch (Exception ex)
        {
