...
namespace RabbitMQ.Client.Mock.NetStandard.Domain
{
    internal abstract class Exchange
    {
        ...
        protected Exchange(string name, string type, int connectionNumber)
        {
            this.Name = name;
            this.Type = type;
        ...
        #region Properties
        protected RabbitMQServer Server => RabbitMQServer.GetInstance(_connectionNumber);
        ...
        public virtual async ValueTask<IList<RabbitQueue>> GetBoundQueuesAsync(string bindingKey)
        {
            ...
            var binding = QueueBindings.TryGetValue(bindingKey, out var result) ? result : null;
            if (binding is null || binding.BoundQueues.Count == 0)
            {
